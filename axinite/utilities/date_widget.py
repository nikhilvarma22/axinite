...
DAYS = [('days','Day')] + [(x,x) for x in range (1, 32)]

this_year = datetime.datetime.now().year
YEARS = [('year', 'Year')] + [(x,x) for x in range(this_year-100, this_year-16)]
...
class DateMultiWidget(forms.MultiWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe(render_to_string('usermanagement/date_widget.html', d))
